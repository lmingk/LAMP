...
parser.add_argument('--train_dataset', type=list, default=['academic','product','yelp' ])
...
def lamp_gnn(args, device):
    ...
    file = open('./model/pretrain_process_for_{}_share{}_lamp{}.csv'.format(test_dataset, args.share_dims,
                                                                               args.syn_graph_num), 'w')
    ...
    pretrain_model = MyGAE(LampContextEncoder(args, args.share_dims, args.output_dims))
    pretrain_model.to(device)

    fake_model = MyGAE(MetaEncoder(args, args.share_dims, args.output_dims))
    fake_model.to(device)

    optimizer_all = torch.optim.Adam([{'params': params, 'lr': 0.0001}], lr=args.model_lr,
                                     weight_decay=5e-4)
    ...
    optimizer_feats = torch.optim.Adam(
        [{'params': feature_params, 'lr': 0.0001}, {'params': pretrain_model.parameters()},
         {'params': fake_model.parameters()}], lr=args.model_lr, weight_decay=5e-4)
    ...
    while True:
        ...
        pretrain_model, loss = lamp_gnn_gradient_step(args, pretrain_model, fake_model, wdiscriminator, syn_graphs,
                                                      subgraphs, MLPs, feature_MLPs, optimizer_wd,
        ...
        if cnt == 50:
            torch.save(optimal_model.encoder.state_dict(),
                       './model/pretrain_encoder_for_{}_share{}_lamp{}.pth'.format(test_dataset, args.share_dims,
                                                                                      args.syn_graph_num))
            for id, name in enumerate(args.train_dataset): subgraphs[name].x = MLPs[name](subgraphs[name].x).detach()
            np.savez('./model/pretrain_graphs_for_{}_share{}_lamp{}.npz'.format(test_dataset, args.share_dims,
                                                                args.syn_graph_num), syn_graphs.get_syn_graphs())
            file.close()
            break
...
def test_graph_lamp(args, device, index, task, ratio):
    ...
    LampEncoder = LampContextEncoder(args, args.share_dims, args.output_dims)

    model = MyGAE(LampEncoder)
    model.encoder.load_state_dict(torch.load(
        './model/pretrain_encoder_for_{}_share{}_lamp{}.pth'.format(dataset, args.share_dims, args.syn_graph_num,)), strict=True)
    if task == 'node':
    ...
    syn_graphs = graph_pool(args=args, addr='./model/pretrain_graphs_for_{}_share{}_lamp{}.npz'.format(dataset,
                                                          args.share_dims,args.syn_graph_num,),device=device)

    tmp = graph.edge_index
    if task == 'link': graph.edge_index = graph.train_edge_index
    mlp = distribution_align(graph, syn_graphs, args, device)
    ...
    if task == 'link':
        fast_weights = model.encoder.reparameterization(mlp(graph.x), graph.train_edge_index, weights)
    else:
        fast_weights = model.encoder.reparameterization(mlp(graph.x), graph.edge_index, weights)
    ...
    if task == 'link':
        optimizer = torch.optim.Adam(
            [{'params': mlp.parameters(), 'lr': 0.001}, {'params': model.encoder.parameters(), 'lr': 0.001}],
            weight_decay=5e-4)
                ...
                if task_name == 'node':
                    ...
                    for node in nodes:
                        for j in range(num_classes):
                            if j == i:
                                pos_link.append((node, j + graph.x.size(0)))
                            else:
                                neg_link.append((node, j + graph.x.size(0)))
                    if len(tmp) == 0:
                        centroids.append(matrix[graph.train_labels].mean(0))
                    else:
                        centroids.append(matrix[graph.train_labels][tmp].mean(0))
                if task_name == 'node_m':
                    tmp = np.where((lab[:, i] == 1).detach().cpu().numpy())[0].tolist()
                    nodes = [graph.train_labels[_] for _ in tmp]
                    for j in graph.train_labels:
                        if j in nodes:
                            pos_link.append((j, i + graph.x.size(0)))
                        else:
                            neg_link.append((j, i + graph.x.size(0)))
                    if len(tmp) == 0:
                        centroids.append(matrix[graph.train_labels].mean(0))
                    else:
                        centroids.append(matrix[graph.train_labels][lab[:, i] == 1].mean(0))
        ...
        optimizer = torch.optim.Adam(
                [{'params': mlp.parameters(), 'lr': 0.001}, {'params': model.encoder.parameters(), 'lr': 0.001},
                 {'params': centroids, 'lr': 0.001}], weight_decay=5e-4)

        return train_graph_by_link(mlp=mlp, model=model, graph=graph, centroids=centroids, optimizer=optimizer,
                                    device=device)
...
test_graph_lamp(args, device, 0, 'node', 0.1)
